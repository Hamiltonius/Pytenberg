...
- Read-only + idempotency (gmail_ledger.jsonl)
...
def banner():
    ...
    print("• Read-only available (--read-only)")
    print("="*70 + "\n")
...
def main():
    ...
    ap.add_argument("--read-only", action="store_true", help="Preview without writing")
    args = ap.parse_args()
    ...
    for idx, m in enumerate(msgs, 1):
        mid = m["id"]
        if mid in seen and not args.read_only:
        ...
        if args.read_only:
            print(f"✓ {idx}/{len(msgs)} {preview} (attachments allowed: {len(allowed)}) [READ-ONLY]")
            continue
        # Write artifacts
